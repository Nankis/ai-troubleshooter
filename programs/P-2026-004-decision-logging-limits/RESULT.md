...
- 新增 `ai_decision_logs` DDL、Go model、memory store 和 MySQL store。
- Orchestrator 记录分类、实体抽取、必要字段检查、工具计划、工具调用、停止查询、总结和失败原因。
...
- 真实 LLM provider 接入后，应把模型原始 trace id、prompt version、completion id 写入 `ai_decision_logs` 或扩展字段。
