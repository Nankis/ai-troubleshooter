...
本系统不允许 Agent 在生产里无限自主循环。Decision Layer 采用“有限工具计划”：先分类、抽取字段、检查必要字段，信息足够后只执行一轮有上限的只读工具查询，再输出需要人工确认的排查结论。当前限制由 Python Agent Platform / Decision Engine 执行。
...
## 上下文隔离与 Context Ledger

为避免一个主 Agent 在长排查里不断吞入原始工具结果导致上下文膨胀，平台增加 `tb_troubleshoot_context_ledger`：

- `case_state`：保存分类后的问题域、问题类型、实体 key、工具预算和失败预算。
- `gateway_tools`：保存 Gateway 当前注册工具的压缩清单和服务分组。
- `knowledge_retrieval`：保存平台经验检索结果数量、候选知识 ID 和置信度摘要。
- `agent_report`：保存 Supervisor、specialist agent 和 Verifier 的动作、原因、风险、工具名和参数 key，不保存大段原始 payload。
- `tool_evidence`：保存每次工具调用的 `summary`、`data_shape`、`result_count` 和 `evidence_refs`，证据引用指向 Gateway `tool_call_id` 或下游 `query_id`。
- `final_summary`：保存最终总结、置信度和最终答案 verifier 结果。

LLM 总结阶段只能接收 `tool_evidence` 的压缩 observation，不能接收 Gateway 原始 `data`。原始工具返回仍在 `tb_troubleshoot_ai_decision_log` 中以脱敏快照方式审计，但不会进入主 Agent 的推理上下文。最终回答前会执行 `verifier_final_answer`：如果没有成功工具证据引用，置信度会降到低位，并要求人工确认。

这套设计把“事实证据”从“模型上下文”里拆出来：Agent 看到的是可审计摘要和引用，业务 Owner 或后续工具可以通过 `tool_call_id` / `query_id` 回查原始证据。
...
- `local_code_inspection`：debug-only 本地代码辅助排查的 repo id、相对路径、命中词、符号、调用边、resolved symbol、receiver type、接口实现关系、行号、行范围、疑点、下一步核对建议和有限脱敏代码摘录；不能写入本地绝对路径、未脱敏源码或大段源码。
- `tool_invocation`：每次工具调用的入参摘要、返回状态、summary、tool call id。
- `tool_query_stopped`：达到失败上限后为什么停止继续查下游。
- `summarize_findings`：如何基于有限证据生成结论。
- `verifier_final_answer`：最终回答是否绑定成功工具证据引用，以及置信度是否被下调。
- `process_failure`：超时或异常时的失败原因。
- `process_skipped`：重复事件、重复处理任务或非入口状态为什么跳过处理。
...
- `MAX_INVESTIGATION_SECONDS`：单个 case 从 Python Agent Platform 开始处理到结束的总时间上限；超时后 case 收敛为 `FAILED`。
...
处理任务侧也有第二道保护：Decision runner 只允许 `NEW`、`NEED_MORE_INFO`、`WAITING_USER_REPLY` 进入处理，并会先把 case 认领到 `READY_TO_INVESTIGATE`。如果重复事件、重复处理任务或终态 case 再次触发处理，系统写入 `process_skipped` 决策日志并返回当前状态，不会再次调用下游工具。

为了避免处理任务异常后 case 永久卡住，Decision runner 使用 `MAX_INVESTIGATION_SECONDS * 2` 作为陈旧窗口，最小 60 秒：
...
这样系统不会让同一个 case 长时间停留在 `INVESTIGATING` 或 `WAITING_TOOL_RESULT`。
