...
- 左侧：问题会话、按服务可折叠分组的已注册 Gateway tools、平台经验沉淀；经验项支持预览、编辑和删除。左侧可独立上下滚动。
- 中间：当前问题对话和 Agent 输出。对话区可独立滚动，输入框固定在底部。
- 右侧：决策层进度、当前状态、工具数量、知识数量和证据来源。右侧可独立上下滚动。

## 输入

- 文字输入：描述生产问题。
- 图片输入：点击 `截图` 选择本地图片，或在输入框聚焦时直接粘贴剪贴板图片。选择或粘贴后会在输入框内预览缩略图，点击缩略图可放大查看，并进入同一个 `images` multipart 字段，和手动选择图片共用后端链路。
