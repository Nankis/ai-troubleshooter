...
Lark / Feishu / Web Chat
  ...
  -> MySQL / Postgres
       -> cases / messages / decisions / audits / root causes
```
...
- 主库继续保存 cases、messages、ai_decision_logs、tool_call_audits、root_causes、knowledge_items。
- 决策层通过结构化条件查历史经验。
...
- `knowledge_items` 超过几百条，关键词检索明显召回不足。
