...
如果没有配置 `GATEWAY_AGENT_CONFIG_FILE` / `GATEWAY_AGENT_CONFIG_JSON`，系统会使用内置默认 agent，便于本地 demo。生产建议总是提供 agent 配置文件。`GATEWAY_AGENT_ID` 控制内置 worker/dev-server 使用哪个 agent id，默认 `business-troubleshooter-v1`。
